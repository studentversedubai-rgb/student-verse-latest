...
export default function Terms() {
  ...
  useEffect(() => {
    // Fetch the terms content from the assets folder
    fetch('/assets/Terms & Conditions SV.txt')
      .then(response => response.text())
      .then(content => {
        // Remove the Termly logo span that creates empty space
        const cleanedContent = content.replace(
          /<span[^>]*background:\s*url\(data:image\/svg\+xml;base64[^>]*><\/span>/gi,
          ''
        );
        setTermsContent(cleanedContent);
        setLoading(false);
      })
      .catch(error => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-white text-lg">Loading Terms & Conditions...</div>
      </div>
    );
  }
